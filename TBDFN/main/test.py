...
# if __name__ == "__main__":
    ...
    Net = MDSiamFLikeNet(n_classes=1)  # model.My_modul1e.Module(3)
    ...
    # 加载模型
    Net.load_state_dict(torch.load('D:\zyf\daima\sss-1\main\our\\uk2-new-1.pth', map_location=device))
    ...
    # 读取所有图片路径
    tests1_path = glob.glob('./data/Landsat/pred/image1/Img5-Aa.png')
    tests2_path = glob.glob('./data/Landsat/pred/image2/Img5-Bb.png')
    tests3_path = glob.glob('./data/Landsat/pred/image3/04.bmp')
    tests4_path = glob.glob('./data/Landsat/pred/image4/04.bmp')
    label_path = glob.glob('./data/Landsat/pred/label/gt-uk1-1.png')
    # 遍历所有图片
    num = 1
    TPSum = 0
    TNSum = 0
    ...
    for tests1_path, tests2_path,tests3_path,tests4_path,label_path in zip(tests1_path, tests2_path, tests3_path, tests4_path, label_path):
        starttime = time.time()
        # 保存结果地址
        ...
        test4_img = cv2.imread(tests4_path)
        ...
        test4_img = test4_img.to(device=device, dtype=torch.float32)

        # 将tensor拷贝到device中：有gpu就拷贝到gpu，否则就拷贝到cpu
        # 预测
        # 使用网络参数，输出预测结果
        pred_Img = Net(test1_img,test2_img,test3_img,test4_img)
        ...
        if num % 10 == 0:
            ...
            OA, Precision, Recall, F1 ,AA = Indicators.ObjectExtract_indicators()
            ...
            print("OA=", str(float('%4f' % OA)), "^^^^^", "mIoU=", str(float('%4f' % IoU)), "^^^^^", "c_mIoU=",
                  ...
                  str(float('%4f' % F1))), "AA=", str(float('%4f' % AA))
    ...
    #       str(float('%4f' % c_IoU)), "^^^^^", "uc_mIoU=", str(float('%4f' % uc_IoU)), "^^^^^", "Precision=",
    #       str(float('%4f' % Precision)), "^^^^^", "Recall=", str(float('%4f' % Recall)), "^^^^^", "mF1=",
    #       str(float('%4f' % F1)), "^^^^^", "Kappa=", str(float('%4f' % Kappa)))
    #
    # f_acc.write(
    #     '==========================================================================================================\n')
    ...
    #     '==========================================================================================================\n')
    # f_acc.write(
    #     '|TotalAcc:|   OA   |   FA   |   MA    |  TE   |  mIoU   |  c_IoU  | uc_IoU  |Precision| Recall  |   F1   |  Kappa  |  AA  \n')
    ...
    print("FP =", FPSum)
    print("FN =", FNSum)
    print("OA =", format(OA, '.4f'))
    print("Kappa =", format(Kappa, '.4f'))
    print("AA =", format(AA, '.4f'))
    print("FA =", format(FA, '.4f'))
    print("MA =", format(MA, '.4f'))
    print("TE =", format(TE, '.4f'))
    print("F1 =", format(F1, '.4f'))


    f_acc.write('\n========== Final Results ==========\n')
    f_acc.write(f'TP={TPSum}\n')
    f_acc.write(f'TN={TNSum}\n')
    f_acc.write(f'FP={FPSum}\n')
    f_acc.write(f'FN={FNSum}\n')
    f_acc.write('OA=' + format(OA, '.4f') + '\n')
    f_acc.write('Kappa=' + format(Kappa, '.4f') + '\n')
    f_acc.write('AA=' + format(AA, '.4f') + '\n')
    f_acc.write('FA=' + format(FA, '.4f') + '\n')
    f_acc.write('MA=' + format(MA, '.4f') + '\n')
    f_acc.write('TE=' + format(TE, '.4f') + '\n')
    f_acc.write('F1=' + format(F1, '.4f') + '\n')
